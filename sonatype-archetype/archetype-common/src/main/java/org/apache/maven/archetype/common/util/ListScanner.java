 ...
 * <p/>
 * <p>
 * These criteria consist of selectors and patterns which have been specified. With the selectors
 ...
 * <p/>
 * <p>
 * The idea is simple. A given directory is recursively scanned for all files and directories. Each
 * file/directory is matched against a set of selectors, including special support for matching
 ...
 * <p/>
 * <p>
 * When no list of include patterns is supplied, "**" will be used, which means that everything will
 * be matched. When no list of exclude patterns is supplied, an empty list is used, such that
 * nothing will be excluded. When no selectors are supplied, none are applied.
 * </p>
 * <p/>
 * <p>
 ...
 * File.separator</code>
 * ('/' under UNIX, '\' under Windows). For example, "abc/def/ghi/xyz.java" is split up in the
 * segments "abc", "def","ghi" and "xyz.java". The same is done for the pattern against which should
 * be matched.
 * </p>
 * <p/>
 * <p>
 * The segments of the name and the pattern are then matched against each other. When '**' is used
 * for a path segment in the pattern, it matches zero or more path segments of the name.
 * </p>
 * <p/>
 * <p>
 * There is a special case regarding the use of <code>File.separator</code>s at the beginning of the
 * pattern and the string to match:<br>
 ...
 * File.separator</code>,
 * the string to match may not start with a <code>File.separator</code>. When one of these rules is
 * not obeyed, the string will not match.
 * </p>
 * <p/>
 * <p>
 ...
 * </p>
 * <p/>
 * <p>"**\*.class" matches all .class files/dirs in a directory tree.
 * </p>
 * <p/>
 * <p>"test\a??.java" matches all files/dirs which start with an 'a', then two more characters and
 * then ".java", in a directory called test.
 * </p>
 * <p/>
 * <p>
 ...
 * <p/>
 * <p>
 * This will scan a directory called test for .class files, but excludes all files in all proper
 * subdirectories of a directory called "modules"
 * </p>
 * <p/>
 * <p>
 * This class was stealed from rg.coudehaus.plexus.util.DirectoryScanner and adapted to search from
 * a List<String>
 * </p>
 * 
...
public class ListScanner
{
    ...
    protected static boolean matchPath(String pattern, String str, boolean isCaseSensitive) {
        return SelectorUtils.matchPath(PathUtils.convertPathForOS(pattern), PathUtils.convertPathForOS(str), isCaseSensitive);
    }
    ...
    protected static boolean matchPatternStart(String pattern, String str, boolean isCaseSensitive) {
        return SelectorUtils.matchPatternStart(PathUtils.convertPathForOS(pattern), PathUtils.convertPathForOS(str), isCaseSensitive);
    }
     ...
     * , so the separator used need not match <code>
     * File.separatorChar</code>.
     * <p/>
     * <p>
    ...
    public List<String> scan(List<String> files) throws IllegalStateException {
        ...
        while (iterator.hasNext()) {
            ...
            // System.err.println("Checking "+(isIncluded ( fileName )?"I":"-")+(isExcluded (
            // fileName )?"E":"-")+fileName);
