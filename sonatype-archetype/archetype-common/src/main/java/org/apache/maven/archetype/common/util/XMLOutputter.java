 ...
 * <p/>
 * Outputs a JDOM document as a stream of bytes. The outputter can manage many styles of document
 * formatting, from untouched to pretty printed. The default is to output the document content
 * exactly as created, but this can be changed by setting a new Format object. For pretty-print
 * output, use <code>{@link Format#getPrettyFormat()}</code>. For whitespace-normalized output, use
 * <code>{@link Format#getCompactFormat()}</code>.
 * <p/>
 * There are <code>{@link #output output(...)}</code> methods to print any of the standard JDOM
 * classes, including Document and Element, to either a Writer or an OutputStream. <b>Warning</b>:
 * When outputting to a Writer, make sure the writer's encoding matches the encoding setting in the
 * Format object. This ensures the encoding in which the content is written (controlled by the
 * Writer configuration) matches the encoding placed in the document's XML declaration (controlled
 * by the XMLOutputter). Because a Writer cannot be queried for its encoding, the information must
 * be passed to the Format manually in its constructor or via the
 * <code>{@link Format#setEncoding}</code> method. The default encoding is UTF-8.
 * <p/>
 * The methods <code>{@link #outputString outputString(...)}</code> are for convenience only; for
 * top performance you should call one of the <code>{@link
 * #output output(...)}</code> methods and pass in your
 * own Writer or OutputStream if possible.
 * <p/>
 * XML declarations are always printed on their own line followed by a line seperator (this doesn't
 * change the semantics of the document). To omit printing of the declaration use
 * <code>{@link Format#setOmitDeclaration}</code>. To omit printing of the encoding in the
 * declaration use <code>{@link Format#setOmitEncoding}</code>. Unfortunatly there is currently no
 * way to know the original encoding of the document.
 * <p/>
 * Empty elements are by default printed as &lt;empty/&gt;, but this can be configured with
 * <code>{@link Format#setExpandEmptyElements}</code> to cause them to be expanded to
 * &lt;empty&gt;&lt;/empty&gt;.
 * 
    ...
    implements Cloneable
{
    ...
    /**
     * Whether output escaping is enabled for the being processed Element - default is
     * <code>true</code>
     */
    ...
    /**
     * This will create an <code>XMLOutputter</code> with the default {@link Format} matching
     * {@link Format#getRawFormat}.
     */
    public XMLOutputter() {}

    /**
     * This will create an <code>XMLOutputter</code> with the specified format characteristics. Note
     * the format object is cloned internally before use.
     */
    ...
    /**
     * This will create an <code>XMLOutputter</code> with all the options as set in the given
     * <code>XMLOutputter</code>. Note that
     * <code>XMLOutputter two = (XMLOutputter)one.clone();</code> would work equally well.
     * 
    ...
    /**
     * Sets the new format logic for the outputter. Note the Format object is cloned internally
     * before use.
     * 
    ...
    /**
     * Returns the current format in use by the outputter. Note the Format object returned is a
     * clone of the one used internally.
     */
    ...
    /**
     * This will print the <code>Document</code> to the given output stream. The characters are
     * printed using the encoding specified in the constructor, or a default of UTF-8.
     * 
    ...
    /**
     * Print out an <code>{@link Element}</code>, including its <code>{@link Attribute}</code>s, and
     ...
     * attributes. This can be useful for printing the content of an element that contains HTML,
     * like "&lt;description&gt;JDOM is &lt;b&gt;fun&gt;!&lt;/description&gt;".
     * 
     * @param element <code>Element</code> to output.
     * @param out <code>OutputStream</code> to use.
     */
    ...
    /**
     * This will handle printing out a list of nodes. This can be useful for printing the content of
     * an element that contains HTML, like "&lt;description&gt;JDOM is
     * &lt;b&gt;fun&gt;!&lt;/description&gt;".
     * 
    ...
    /**
     * Print out a <code>{@link Text}</code> node. Perfoms the necessary entity escaping and
    ...
    /**
     * Get an OutputStreamWriter, using prefered encoding (see {@link Format#setEncoding}).
     */
     ...
     * <p/>
     * <p/>
     * Warning: using your own Writer may cause the outputter's preferred character encoding to be
     * ignored. If you use encodings other than UTF-8, we recommend using the method that takes an
    ...
    /**
     * Return a string representing a document. Uses an internal StringWriter. Warning: a String is
     * Unicode, which may not match the outputter's specified encoding.
     * 
     * @param doc <code>Document</code> to format.
     */
    ...
    /**
     * Return a string representing a DocType. Warning: a String is Unicode, which may not match the
    ...
    /**
     * Return a string representing an element. Warning: a String is Unicode, which may not match
    ...
    /**
     * Return a string representing a list of nodes. The list is assumed to contain legal JDOM
     * nodes.
     * 
    ...
    /**
     * Return a string representing a CDATA node. Warning: a String is Unicode, which may not match
     * the outputter's specified encoding.
     * 
    ...
    /**
     * Return a string representing a Text node. Warning: a String is Unicode, which may not match
     * the outputter's specified encoding.
     * 
    ...
    /**
     * Return a string representing a comment. Warning: a String is Unicode, which may not match the
     * outputter's specified encoding.
     * 
    ...
    /**
     * Return a string representing a PI. Warning: a String is Unicode, which may not match the
     * outputter's specified encoding.
     * 
    ...
    /**
     * Return a string representing an entity. Warning: a String is Unicode, which may not match the
     * outputter's specified encoding.
     * 
    ...
    /**
     * This will handle printing of the declaration. Assumes XML version 1.0 since we don't directly
     * know.
     * 
     ...
     * <code>&amp;entity;</code> will be printed. However, subclasses are free to override this
     * method to print the contents of the entity instead.
     * 
     * @param entity <code>EntityRef</code> to output.
     * @param out <code>Writer</code> to use.
     */
    ...
    /**
     * This will handle printing of content within a given range. The range to print is specified in
     * typical Java fashion; the starting index is inclusive, while the ending index is exclusive.
     * 
     ...
     * <code>{@link Text}</code> nodes. It is an error to have any other pass this method any other
     * type of node.
     * 
     * @param content <code>List</code> of content to output
     * @param start index of first content node (inclusive).
    ...
    /**
     * This will print indents (only if the newlines flag was set to <code>true</code>, and indent
     * is non-null).
     * 
     * @param out <code>Writer</code> to use
     ...
     * to the appropriate entity reference, suitable for XML attributes. It does not convert the
     * single quote (') because it's not necessary as the outputter writes attributes surrounded by
     * double-quotes.
     * 
    ...
    public String toString() {
        ...
        return ("XMLOutputter[omitDeclaration = " + userFormat.omitDeclaration + ", " + "encoding = " + userFormat.encoding + ", " + "omitEncoding = " + userFormat.omitEncoding + ", " + "indent = '"
                + userFormat.indent + "'" + ", " + "expandEmptyElements = " + userFormat.expandEmptyElements + ", " + "lineSeparator = '" + buffer.toString() + "', " + "textMode = " + userFormat.mode + "]");
    }

    /**
     * Factory for making new NamespaceStack objects. The NamespaceStack created is actually an
     * inner class extending the package protected NamespaceStack, as a way to make NamespaceStack
     * "friendly" toward subclassers.
     */
    ...
    /**
     * Our own null subclass of NamespaceStack. This plays a little trick with Java access
     * protection. We want subclasses of XMLOutputter to be able to override protected methods that
     * declare a NamespaceStack parameter, but we don't want to declare the parent NamespaceStack
     * class as public.
     */
    ...
    /*
     * The methods below here are deprecations of protected methods. We don't usually deprecate
     * protected methods, so they're commented out. They're left here in case this mass deprecation
     * causes people trouble. Since we're getting close to 1.0 it's actually better for people to
